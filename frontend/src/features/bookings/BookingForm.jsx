...
export default function BookingForm() {
  ...
  const passengerTypes = useMemo(() => {
    ...
    const childCount = Math.min(
      9 - adultCount,
      Math.max(0, Number(searchParams.get("childCount")) || 0),
    );
      ...
      <section className="border-b border-surface-container-high bg-primary">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-container-padding py-stack-lg text-on-primary md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-headline-lg font-headline-lg">
              Hoàn tất thông tin chuyến bay
            </h1>
            <p className="mt-2 max-w-2xl text-body-md font-body-md text-primary-fixed">
              Kiểm tra hành khách và dịch vụ đi kèm trước khi chọn ghế.
            </p>
          </div>
          <button
    ...
    <section className="rounded-xl border border-surface-container-high bg-surface-container-lowest p-stack-md shadow-[0_4px_12px_rgba(26,54,93,0.05)]">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="grid grid-cols-3 gap-3 rounded-lg bg-surface p-3 text-center">
          <Metric label="Khởi hành" value={formatTime(flight.departureTime)} />
          <Metric label="Thời lượng" value={flight.duration ? `${flight.duration} phút` : "-"} />
    ...
    <div className="grid grid-cols-1 gap-stack-lg xl:grid-cols-2">
      {Array.from({ length: passengerCount }, (_, index) => (
        <OptionPanel
          icon={Briefcase}
          key={`baggage-${index}`}
          title={`Hành lý · Hành khách ${index + 1}`}
        >
          <RadioList
            emptyLabel="Không chọn hành lý ký gửi"
            items={baggageOptions}
            itemLabel={(item) =>
              `${item.weight_kg}kg${item.description ? ` · ${item.description}` : ""}`
            }
            onChange={(value) => onBaggageChange(index, value)}
            selectedItem={baggage[index]}
          />
        </OptionPanel>
      ))}
      {Array.from({ length: passengerCount }, (_, index) => (
        <OptionPanel
          icon={Utensils}
          key={`meal-${index}`}
          title={`Suất ăn · Hành khách ${index + 1}`}
        >
          <RadioList
            emptyLabel="Không chọn suất ăn"
            items={mealOptions}
            itemLabel={(item) => item.name}
            onChange={(value) => onMealChange(index, value)}
            selectedItem={meal[index]}
          />
        </OptionPanel>
      ))}
      <OptionPanel icon={BadgePercent} title="Mã giảm giá">
        ...
        <div
          className={cn(
            ...
            isDiscountApplied ? "text-status-success" : "text-on-surface-variant",
          )}
        >
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        type="button"
      >
        <span className="text-body-sm font-body-sm">{emptyLabel}</span>
        <span className="font-data-mono text-data-mono">0 VND</span>
      </button>
      {items.map((item) => (
        <button
          className={cn(
          ...
          type="button"
        >
          <span className="text-body-sm font-body-sm">{itemLabel(item)}</span>
          <span className="font-data-mono text-data-mono">{formatCurrency(item.price)}</span>
        </button>
      ))}
    </div>
  );
...
function BookingEstimate({ flight, baggage, meal, total }) {
  ...
  return (
    <aside className="h-fit rounded-xl border border-surface-container-high bg-surface-container-lowest p-stack-lg shadow-sm lg:col-span-4 lg:sticky lg:top-24">
      <h2 className="text-title-lg font-title-lg text-primary">Tóm tắt dự kiến</h2>
      ...
      <p className="mt-3 text-body-sm text-on-surface-variant">
        Tổng tiền cuối cùng sẽ được tính an toàn trên máy chủ sau khi chọn ghế.
      </p>
    </aside>
  );
}
...
function createPassenger(passengerType) {
  return {
    title: passengerType === "child" ? "Cô" : "Ông",
    lastName: "",
    firstName: "",
    birthDate: "",
