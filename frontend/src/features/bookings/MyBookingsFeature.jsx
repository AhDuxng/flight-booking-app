...
const statusStyles = {
  ...
  refund_pending: "bg-status-warning/10 text-[#B45309] border-status-warning/30",
};
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-stack-lg px-container-padding py-stack-lg lg:grid-cols-12">
        <div className="flex min-w-0 flex-col gap-stack-md lg:col-span-9">
            ...
            <div className="flex flex-col gap-stack-md md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <p className="font-body-md text-body-md text-on-surface-variant">
                  Theo dõi trạng thái đặt chỗ và thanh toán cho các chuyến bay của bạn.
                </p>
              </div>
              <SelectField
                label="Trạng thái"
          ...
          <label className="relative block">
            ...
            <input
              className="h-12 w-full rounded-lg border border-surface-container-highest bg-surface-container-lowest pl-12 pr-4 font-body-md text-body-md text-on-surface outline-none transition focus:ring-2 focus:ring-primary/20"
...
function SelectField({ label, value, options, onChange }) {
  return (
    <label className="relative block w-full sm:w-auto">
      ...
      <select
        className="h-10 w-full min-w-[180px] appearance-none rounded border border-outline-variant bg-surface-container-lowest px-4 pr-10 text-body-sm text-on-surface outline-none focus:ring-2 focus:ring-primary/20"
...
function BookingCard({ booking }) {
  ...
  return (
    <article
      className={cn(
        "overflow-hidden rounded-lg border border-surface-container-highest bg-surface-container-lowest p-stack-md flight-card-shadow",
        isCancelled && "opacity-75",
      )}
    >
      <div className="flex flex-col gap-stack-md md:flex-row md:items-center md:justify-between">
        <div className="flex flex-1 flex-col gap-stack-md md:flex-row md:items-center">
          <div className="md:w-44">
            <span className="block text-body-sm text-on-surface-variant">
              {formatDateTime(flight?.departure_time)}
            </span>
            <span className="mt-1 inline-flex rounded bg-primary/5 px-2 py-1 font-data-mono text-data-mono text-primary">
              {booking.id}
            </span>
          </div>
          <div className="grid min-w-0 flex-1 grid-cols-[auto_minmax(50px,1fr)_auto] items-center gap-stack-sm">
            <div className="text-center">
              <p className="text-title-lg text-primary">{flight?.origin_airport?.code ?? "-"}</p>
            ...
            <div className="relative">
              ...
              <Plane className="absolute left-1/2 top-1/2 h-5 w-5 -translate-x-1/2 -translate-y-1/2 rotate-90 bg-surface-container-lowest text-outline" />
            </div>
            <div className="text-center">
          ...
          <div className="flex gap-4 text-body-sm md:w-48 md:flex-col md:gap-1">
        ...
        <div className="flex items-center gap-3 md:w-40 md:flex-col">
          <span
            className={cn(
              "inline-flex rounded-full border px-2.5 py-1 text-label-md",
          ...
          <Link
            className={cn(
              "w-full rounded px-4 py-2 text-center text-label-md transition-colors",
              isPending
                ? "bg-status-warning font-bold text-primary hover:bg-secondary-container"
...
function EmptyBookings() {
  return (
    <div className="rounded-lg border border-surface-container-highest bg-surface-container-lowest p-stack-lg text-center flight-card-shadow">
      ...
      <h2 className="mb-2 text-title-lg text-primary">Chưa có đơn hàng phù hợp</h2>
      <p className="mb-5 text-body-md text-on-surface-variant">
        Tìm chuyến bay mới để bắt đầu hành trình tiếp theo.
      </p>
      <Link
        className="inline-flex h-10 items-center justify-center rounded bg-primary px-4 text-label-md text-on-primary"
        to="/"
      >
        Tìm chuyến bay
      </Link>
...
function SummaryPanel({ bookings }) {
  ...
  return (
    <aside className="flex flex-col gap-stack-md lg:col-span-3">
      <section className="rounded-lg border border-surface-container-highest bg-surface-container-lowest p-stack-md flight-card-shadow">
        <h2 className="mb-stack-md text-title-lg text-on-surface">Tổng quan</h2>
        <SummaryRow icon={ReceiptText} label="Tổng đơn" value={bookings.length} />
        <SummaryRow icon={CreditCard} label="Chờ thanh toán" value={pendingCount} />
        <SummaryRow icon={XCircle} label="Đã hủy" value={cancelledCount} />
      </section>
    </aside>
  );
}
...
function SummaryRow({ icon: Icon, label, value }) {
  return (
    <div className="mb-2 flex items-center justify-between rounded bg-surface-container-low px-3 py-2 last:mb-0">
