  ...
  useEffect(() => {
    const intervalId = window.setInterval(async () => {
      try {
        ...
        setSeats(nextSeats);
        setSelectedSeats((currentSeats) => currentSeats.filter((currentSeat) => nextSeats.some((seat) => seat.id === currentSeat.id && seat.status === "available")));
      } catch {
        return;
      }
    }, 15_000);
  ...
  const handleContinue = async () => {
    ...
    try {
      const response = await bookingService.create({
        ...
        baggage: (Array.isArray(passengerInfo.baggage) ? passengerInfo.baggage : []).map((item, passengerIndex) => item ? { passengerIndex, baggageOptionId: item.id, quantity: 1 } : null).filter(Boolean),
        meals: (Array.isArray(passengerInfo.meal) ? passengerInfo.meal : []).map((item, passengerIndex) => item ? { passengerIndex, mealOptionId: item.id, quantity: 1 } : null).filter(Boolean),
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-stack-lg px-container-padding py-stack-lg">
        ...
        <div className="grid grid-cols-1 items-start gap-stack-lg lg:grid-cols-12">
          <div className="flex flex-col gap-stack-lg lg:col-span-8">
            <FlightHeader flight={flight} passengerCount={passengerCount} selectedCount={selectedSeats.length} />
            <SeatLegend />
            <SeatMap seats={seats} onSelectSeat={handleSelectSeat} selectedSeats={selectedSeats} />
          </div>
          <div className="lg:sticky lg:top-24 lg:col-span-4"><SeatSummary flight={flight} isSubmitting={isSubmitting} onContinue={handleContinue} passengerCount={passengerCount} selectedSeats={selectedSeats} totalPrice={totalPrice} /></div>
        </div>
      </div>
    </div>
  );
...
function SeatSummary({ flight, selectedSeats, passengerCount, totalPrice, onContinue, isSubmitting }) {
  const seatTotal = selectedSeats.reduce((total, seat) => total + Number(seat.price ?? 0), 0);
  const isComplete = selectedSeats.length === passengerCount;
  return <aside className="relative flex flex-col gap-stack-md overflow-hidden rounded-xl border border-surface-variant bg-surface-container-lowest p-container-padding shadow-md"><div className="absolute left-0 top-0 h-1 w-full bg-primary" /><h2 className="text-title-lg font-title-lg text-primary">Chi tiết đặt chỗ</h2><div className="flex items-center justify-between border-b border-outline-variant py-3"><div><span className="block text-label-md font-label-md text-on-surface-variant">Chuyến bay</span><span className="font-data-mono text-data-mono font-bold text-on-surface">{flight.origin} - {flight.destination}</span></div><div className="max-w-36 text-right text-body-sm text-on-surface">{formatDateTime(flight.departureTime)}</div></div><SelectedSeatPanel selectedSeats={selectedSeats} /><div className="flex flex-col gap-2 border-t border-outline-variant pt-4"><PriceLine label="Ghế đã chọn" value={seatTotal} /><PriceLine label="Dịch vụ bổ sung" value={totalPrice - seatTotal} /></div><div className="flex items-end justify-between border-t border-outline-variant pt-4"><span className="text-title-lg font-title-lg text-primary">Tạm tính</span><span className="text-headline-md font-headline-md font-bold text-primary">{formatCurrency(totalPrice)}</span></div><div className="flex items-start gap-2 rounded-lg bg-primary-fixed p-3 text-on-primary-fixed"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><p className="text-body-sm font-body-sm">Ghế sẽ được giữ sau khi hệ thống tạo booking thành công.</p></div><button className={cn("mt-1 flex w-full items-center justify-center gap-2 rounded-xl bg-status-warning py-4 text-label-md font-label-md font-bold text-primary transition-all hover:bg-secondary-container", (!isComplete || isSubmitting) && "cursor-not-allowed opacity-50")} disabled={!isComplete || isSubmitting} onClick={onContinue} type="button">{isSubmitting ? "Đang tạo đặt chỗ..." : "Tiếp tục thanh toán"}<ArrowRight className="h-4 w-4" /></button></aside>;
}
...
function SelectedSeatPanel({ selectedSeats }) {
  ...
  return <div className="grid gap-2">{selectedSeats.map((seat, index) => <div className="flex min-h-14 items-center justify-between rounded-lg bg-primary p-3 text-on-primary shadow-md" key={seat.id}><div><span className="block font-data-mono text-lg font-bold">Hành khách {index + 1} · Ghế {seat.code}</span><span className="text-xs opacity-80">{seat.zone}</span></div><Plane className="h-5 w-5 text-status-warning" /></div>)}</div>;
}
