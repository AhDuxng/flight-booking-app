...
export default function FlightStatusFeature() {
  ...
  const submit = async (event) => {
    ...
    setLoading(true);
    try {
      const response = await operationService.getFlightStatus({
        flightNumber,
  ...
  return (
    <div className="min-h-[70vh] bg-surface-container">
      <section className="bg-primary px-container-padding py-section-gap text-on-primary">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mt-2 text-primary-fixed">
            Giờ dự kiến/thực tế, cửa ra máy bay và băng chuyền hành lý được cập nhật tại đây.
          </p>
          <form
            className="mx-auto mt-6 grid max-w-2xl gap-3 rounded-xl bg-surface-container-lowest p-4 text-on-surface shadow-lg sm:grid-cols-[1fr_1fr_auto]"
            onSubmit={submit}
          >
            <input
              className="h-11 rounded-lg border border-outline-variant px-3 uppercase"
              onChange={(e) => setFlightNumber(e.target.value)}
              placeholder="VN123"
              value={flightNumber}
            />
            <input
              className="h-11 rounded-lg border border-outline-variant px-3"
              onChange={(e) => setDepartureDate(e.target.value)}
              type="date"
              value={departureDate}
            />
            <button
              className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-secondary-container px-5 font-semibold text-on-secondary-container"
              type="submit"
            >
      ...
      <div className="mx-auto max-w-5xl px-container-padding py-stack-lg">
        {loading ? (
          <Loading label="Đang tra cứu" />
        ) : flights.length ? (
          flights.map((flight) => <FlightStatusCard flight={flight} key={flight.id} />)
        ) : searched ? (
          <EmptyState
            icon={PlaneTakeoff}
            title="Không tìm thấy chuyến bay"
            description="Kiểm tra lại mã chuyến hoặc ngày khởi hành."
          />
        ) : null}
      </div>
    </div>
...
function FlightStatusCard({ flight }) {
  const latest = [...(flight.flight_status_events ?? [])].sort(
  ...
  return (
    <article className="mb-4 rounded-xl border border-outline-variant bg-surface-container-lowest p-container-padding shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-outline-variant pb-4">
        ...
        <span className="rounded-full bg-primary-fixed px-3 py-1 text-label-md font-semibold uppercase text-primary">
          {flight.status}
        </span>
      </div>
      <div className="mt-4 grid gap-4 sm:grid-cols-3">
        <Info
          ...
          value={formatDateTime(
            flight.actual_departure_time ||
              latest?.estimated_departure_time ||
        ...
        <Info
          icon={MapPin}
          label="Cửa / Nhà ga"
          value={`${latest?.gate || flight.gate || "-"} / ${latest?.terminal || flight.terminal || "-"}`}
        />
        <Info
          icon={MapPin}
          label="Băng chuyền"
          value={latest?.baggage_carousel || flight.baggage_carousel || "Chưa cập nhật"}
        />
      </div>
      {latest?.message || flight.delay_reason ? (
        <p className="mt-4 rounded-lg bg-surface-container p-3 text-body-sm text-on-surface-variant">
          {latest?.message || flight.delay_reason}
        </p>
      ) : null}
    </article>
