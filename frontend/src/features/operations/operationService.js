...
export const operationService = {
  ...
  addSupportMessage: (ticketId, body) =>
    api.post(`/operations/support-tickets/${ticketId}/messages`, { body }),
  addAdminSupportMessage: (ticketId, body, isInternal = false) =>
    api.post(`/operations/admin/support/${ticketId}/messages`, { body, isInternal }),
  checkIn: (bookingId, passengerIds, seatAssignments = []) =>
    api.post(`/operations/bookings/${bookingId}/check-in`, {
      passengerIds,
  ...
  confirmChange: (requestId, provider = "cash") => {
    ...
    return api.post(
      `/operations/change-quotes/${requestId}/confirm`,
      { provider },
      {
  ...
  decideRefund: (refundId, payload) =>
    api.post(`/operations/admin/refunds/${refundId}/decision`, payload),
  downloadBoardingPass: (checkInId) =>
    download(`/operations/boarding-passes/${checkInId}/pdf`, `boarding-pass-${checkInId}.pdf`),
  downloadETicket: (bookingId) =>
    download(`/operations/bookings/${bookingId}/e-ticket.pdf`, `vietfly-${bookingId}.pdf`),
  ...
  quoteChange: (bookingId, newFlightId) =>
    api.post(`/operations/bookings/${bookingId}/change-quotes`, { newFlightId }),
  setBookingFare: (bookingId, fareId) =>
    api.patch(`/operations/bookings/${bookingId}/fare`, { fareId }),
  updateAdminResource: (resource, id, payload) =>
    api.patch(`/operations/admin/${resource}/${id}`, payload),
};
