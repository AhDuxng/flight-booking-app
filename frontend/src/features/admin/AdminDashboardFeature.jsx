...
export default function AdminDashboardFeature() {
  ...
  const loadDashboard = async () => {
    setIsLoading(true);
    try {
      const [dashboardResponse, bookingsResponse, flightsResponse] = await Promise.all([
        adminService.getDashboard(),
        adminService.getBookings({ limit: 3 }),
        adminService.getFlights({ limit: 3 }),
      ]);
  ...
  const stats = [
    ...
    { label: "Tổng đơn đặt vé", value: dashboard.bookings, trend: "Dữ liệu thực" },
    {
      label: "Chuyến bay hoạt động",
      value: dashboard.scheduledFlights,
      trend: `${dashboard.flights} tổng chuyến`,
    },
    {
      label: "Đơn chờ thanh toán",
      value: dashboard.pendingBookings,
      trend: `${dashboard.confirmedBookings} đã xác nhận`,
    },
  ];
  ...
  return (
    <div className="flex min-w-0 flex-col gap-stack-md">
      <AdminPageHeader
        action={
          <Button as={Link} icon={Plane} to="/admin/flights/create" variant="admin">
            Tạo chuyến bay
          </Button>
        }
        description="Tổng quan vận hành bán vé, lịch bay và các giao dịch cần theo dõi."
  ...
  return (
    <section className="flex min-w-0 flex-col gap-stack-sm">
      <div className="flex items-center justify-between">
        ...
        <Link
          className="rounded-lg bg-primary px-3 py-1.5 text-label-md text-on-primary shadow-sm transition-colors hover:bg-primary-container"
          to={to}
        >
          Xem tất cả
        </Link>
      ...
      {rows.length ? (
        ...
        <div className="rounded-lg border border-outline-variant bg-surface-container-lowest p-stack-md text-body-sm text-on-surface-variant">
          Chưa có dữ liệu.
        </div>
      )}
    </section>
