...
export default function AdminUserDetailFeature() {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-stack-md">
      <AdminPageHeader
        description="Xem hồ sơ người dùng và các hoạt động liên quan."
        title={user.full_name || "Người dùng chưa đặt tên"}
      />
      <section className="grid grid-cols-1 gap-stack-md lg:grid-cols-3">
        <article className="rounded-lg border border-surface-container-high bg-surface-container-lowest p-stack-md shadow-sm">
          ...
          <h2 className="text-title-lg text-primary">{user.full_name || "Chưa cập nhật"}</h2>
          ...
          <p className="mt-2 text-body-md text-on-surface-variant">
            Quốc tịch: {user.nationality || "Chưa cập nhật"}
          </p>
          <span className="mt-stack-md inline-flex items-center gap-1 rounded-full border border-primary-fixed bg-primary-fixed px-2.5 py-0.5 text-label-md text-primary">
            <ShieldCheck className="h-3.5 w-3.5" />
            {user.role === "admin" ? "Quản trị viên" : "Tài khoản khách hàng"}
          </span>
        </article>
        <article className="rounded-lg border border-surface-container-high bg-surface-container-lowest p-stack-md shadow-sm lg:col-span-2">
          <h2 className="mb-stack-md text-title-lg text-primary">Đơn đặt vé liên quan</h2>
          <div className="grid gap-stack-sm">
            {bookings.length ? (
              bookings.map((booking) => (
                ...
                >
                  <div>
                    <p className="font-data-mono text-label-md text-primary">{booking.id}</p>
                    <p className="text-body-sm text-on-surface-variant">
                      {booking.flight?.flight_number ?? "-"} · {booking.status}
                    </p>
                  </div>
              ...
              <p className="rounded-lg bg-surface-container-low p-3 text-body-md text-on-surface-variant">
                Người dùng này chưa có đơn đặt vé.
              </p>
            )}
          </div>
        </article>
        <ActivityPanel
          ...
          render={(item) => `${item.rating}/5 · ${item.comment || "Không có nhận xét"}`}
        />
        <ActivityPanel
          items={user.notifications}
          title="Thông báo gần đây"
          render={(item) => `${item.title} · ${item.read_at ? "Đã đọc" : "Chưa đọc"}`}
        />
      </section>
    </div>
  );
...
function ActivityPanel({ items = [], title, render }) {
  return (
    <article className="rounded-lg border border-surface-container-high bg-surface-container-lowest p-stack-md shadow-sm lg:col-span-3">
      <h2 className="mb-stack-md text-title-lg text-primary">{title}</h2>
      <div className="grid gap-2">
        {items.length ? (
          items.map((item) => (
            <div
              className="rounded-lg bg-surface-container-low p-3 text-body-sm text-on-surface-variant"
              key={item.id}
            >
              {render(item)}
            </div>
