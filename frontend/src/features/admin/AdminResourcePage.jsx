...
const resourceConfig = {
  flights: {
    ...
    map: (item) => ({
      ...
      route: `${item.origin_airport?.code ?? "-"} - ${item.destination_airport?.code ?? "-"}`,
      departure: formatDateTime(item.departure_time),
    ...
    columns: [
      ...
      { key: "status", label: "Trạng thái" },
      {
        ...
        render: (row) => (
          <Link
            className="inline-flex items-center justify-center rounded-lg bg-primary px-3 py-1.5 text-label-md text-on-primary shadow-sm transition-colors hover:bg-primary-container"
  ...
  bookings: {
    ...
    placeholder: "Tìm theo mã đặt chỗ hoặc chuyến bay",
    load: () => adminService.getBookings({ limit: 100 }),
    map: (item) => ({
      id: item.id,
      customer: item.user?.full_name ?? item.contact_email,
      flight: item.flight?.flight_number ?? "-",
      flightStatus: item.flight?.status,
      departure: formatDateTime(item.flight?.departure_time),
      departureTime: item.flight?.departure_time,
  ...
  payments: {
    title: "Thanh toán",
    description: "Kiểm tra giao dịch và trạng thái đối soát.",
    placeholder: "Tìm theo mã giao dịch hoặc booking",
    load: () => adminService.getPayments({ limit: 100 }),
    map: (item) => ({
      id: item.id,
      reference: item.transaction_ref,
      booking: item.booking?.id ?? item.booking_id,
    ...
    columns: [
      ...
      { key: "provider", label: "Phương thức" },
  ...
  reviews: {
    title: "Đánh giá",
    description: "Theo dõi phản hồi sau chuyến bay.",
    placeholder: "Tìm theo khách hàng hoặc chuyến bay",
    load: () => adminService.getReviews({ limit: 100 }),
    map: (item) => ({
      id: item.id,
      customer: item.user?.full_name ?? "-",
      flight: item.flight?.flight_number ?? "-",
      rating: `${item.rating}/5`,
      visibility: item.is_visible ? "Hiển thị" : "Đã ẩn",
  ...
  users: {
    title: "Người dùng",
    description: "Quản lý hồ sơ tài khoản khách hàng.",
    placeholder: "Tìm theo tên, số điện thoại hoặc mã người dùng",
    load: () => adminService.getUsers({ limit: 100 }),
    map: (item) => ({
      id: item.id,
      name: item.full_name ?? "-",
      phone: item.phone ?? "-",
  ...
  airports: {
    title: "Sân bay",
    description: "Quản lý sân bay và thành phố đang khai thác.",
    placeholder: "Tìm theo mã sân bay, thành phố hoặc tên",
    load: () => adminService.getAirports(),
    map: (item) => ({
      id: item.id,
      code: item.code,
      name: item.name,
  ...
  airlines: {
    title: "Hãng bay",
    description: "Theo dõi hãng hàng không đang khai thác.",
    placeholder: "Tìm theo tên hãng hoặc mã",
    load: () => adminService.getAirlines(),
    map: (item) => ({
      id: item.id,
      code: item.code,
      name: item.name,
      country: item.country,
      active: item.is_active ? "Đang hoạt động" : "Ngừng hoạt động",
    }),
    columns: [
  ...
  aircrafts: {
    title: "Tàu bay",
    description: "Quản lý đội tàu bay và số ghế khai thác.",
    placeholder: "Tìm theo mã hoặc dòng máy bay",
    load: () => adminService.getAircrafts(),
    map: (item) => ({
      id: item.id,
      code: item.code,
      model: item.model,
...
export default function AdminResourcePage({ resource }) {
    ...
    try {
      ...
      toast.success(
        status === "success"
          ...
          : "Đã từ chối thanh toán tiền mặt.",
      );
      await loadRows();
    } catch (requestError) {
  ...
  const handleCancelBooking = async (row) => {
    if (
      !window.confirm(
        `Hủy booking ${row.id}? Nếu đã thanh toán, booking sẽ chuyển sang chờ hoàn tiền.`,
      )
    ) {
      return;
    }
    ...
    if (resource === "users") {
      return [
        ...config.columns,
        {
          ...
          label: "",
          render: (row) => (
            <Link className={adminActionClass} to={`/admin/users/${row.id}`}>
              Chi tiết
            </Link>
          ),
        },
        ...
        return [
          ...config.columns,
          {
            ...
            render: (row) => (
              <button
                className={adminActionClass}
                onClick={() => handleToggleReview(row)}
                type="button"
              >
            ...
            render: (row) =>
              row.status === "refund_pending" ? (
                ...
                <div className="flex gap-2">
                  <button
                    className={adminActionClass}
                    onClick={() => handleCashPayment(row, "success")}
                    type="button"
                  >
                    Xác nhận
                  </button>
                  <button
                    className={adminActionClass}
                    onClick={() => handleCashPayment(row, "failed")}
                    type="button"
                  >
                    Từ chối
                  </button>
    ...
    return [
      ...config.columns,
      {
        ...
        render: (row) => (
          <button
            className={adminActionClass}
            onClick={() => setEditingItem(row._raw)}
            type="button"
          >
  ...
  const headerAction =
    resource === "flights" ? (
      <Button as={Link} icon={Plus} to="/admin/flights/create" variant="admin">
        Tạo chuyến bay
      </Button>
    ) : masterResource ? (
      <Button icon={Plus} onClick={() => setEditingItem(null)} variant="admin">
        Thêm mới
      </Button>
    ) : null;
  ...
  return (
    <div className="flex min-w-0 flex-col gap-stack-md">
      ...
      />
      {isLoading ? (
        <Loading className="min-h-48" label="Đang tải dữ liệu" />
      ) : error ? (
        <ErrorMessage message={error} onRetry={loadRows} />
      ) : filteredRows.length > 0 ? (
        <AdminDataTable columns={columns} rows={filteredRows} />
      ) : (
        <EmptyState
          description="Thử thay đổi từ khóa tìm kiếm hoặc kiểm tra lại dữ liệu."
...
const fieldClass =
  "h-11 w-full rounded-lg border border-outline-variant bg-surface px-3 text-body-md text-on-surface outline-none focus:border-primary focus:ring-2 focus:ring-primary/20";
const adminActionClass =
  "inline-flex items-center justify-center gap-1 rounded-lg bg-primary px-3 py-1.5 text-label-md text-on-primary shadow-sm transition-colors hover:bg-primary-container disabled:pointer-events-none disabled:opacity-50";
...
function MasterDataModal({ item, onClose, onSaved, resource }) {
    ...
    if (isOpen && resource === "aircrafts") {
      adminService
        ...
        .then((response) => {
          ...
          setValues((current) => ({
            ...current,
            airlineId: current.airlineId || items[0]?.id || "",
          }));
        })
        .catch(() => setAirlines([]));
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const action = item
        ? resource === "airports"
          ? adminService.updateAirport(item.id, payload)
          : resource === "airlines"
            ? adminService.updateAirline(item.id, payload)
            : adminService.updateAircraft(item.id, payload)
        : resource === "airports"
          ? adminService.createAirport(payload)
          : resource === "airlines"
            ? adminService.createAirline(payload)
            : adminService.createAircraft(payload);
      await action;
  ...
  return (
    <Modal isOpen onClose={onClose} title={item ? "Cập nhật dữ liệu" : "Thêm dữ liệu mới"}>
      <form className="grid gap-4" onSubmit={handleSubmit}>
        {resource === "aircrafts" ? (
          <Field label="Hãng bay">
            <select
              className={fieldClass}
              onChange={(event) => setField("airlineId", event.target.value)}
              required
        ...
        <Field
          label={
            resource === "airports" ? "Mã IATA" : resource === "airlines" ? "Mã hãng" : "Mã tàu bay"
          }
        >
          <input
            className={fieldClass}
            maxLength={resource === "airports" ? 3 : 20}
          ...
          <>
            ...
            <Field label="Tổng số ghế">
              <input
                ...
                max="1000"
                min="1"
                onChange={(event) => setField("totalSeats", event.target.value)}
                required
                type="number"
        ...
        <div className="flex justify-end gap-3">
          <Button onClick={onClose} type="button" variant="admin">
            Hủy
          </Button>
          <Button disabled={isSaving} type="submit" variant="admin">
