...
export default function AdminResourcePage({ resource }) {
  ...
  const columns = useMemo(() => {
    if (resource === "users") {
      return [...config.columns, { key: "actions", label: "", render: (row) => <Link className="text-label-md text-primary hover:underline" to={`/admin/users/${row.id}`}>Chi tiết</Link> }];
    }
    if (!masterResource) {
      if (resource === "bookings") {
        return [...config.columns, { key: "actions", label: "", render: (row) => ["pending", "paid", "confirmed"].includes(row.status) && new Date(row.departureTime).getTime() > Date.now() && ["scheduled", "delayed"].includes(row.flightStatus) ? <button className="text-label-md text-status-error hover:underline" onClick={() => handleCancelBooking(row)} type="button">Hủy</button> : "" }];
      }
      if (resource === "reviews") {
        return [...config.columns, { key: "actions", label: "", render: (row) => <button className="text-label-md text-primary hover:underline" onClick={() => handleToggleReview(row)} type="button">{row.isVisible ? "Ẩn" : "Hiện"}</button> }];
      }
      if (resource === "payments") {
        return [...config.columns, { key: "actions", label: "", render: (row) => row.status === "refund_pending" ? <button className="text-label-md text-primary hover:underline" onClick={() => handleRefund(row)} type="button">Hoàn tiền</button> : row.provider === "cash" && row.status === "pending" ? <div className="flex gap-2"><button className="text-label-md text-status-success hover:underline" onClick={() => handleCashPayment(row, "success")} type="button">Xác nhận</button><button className="text-label-md text-status-error hover:underline" onClick={() => handleCashPayment(row, "failed")} type="button">Từ chối</button></div> : "" }];
      }
      return config.columns;
    }
    return [...config.columns, { key: "actions", label: "", render: (row) => <button className="inline-flex items-center gap-1 text-label-md text-primary hover:underline" onClick={() => setEditingItem(row._raw)} type="button"><Pencil className="h-4 w-4" />Sửa</button> }];
  }, [config.columns, masterResource, resource, rows]);

  const headerAction = resource === "flights"
    ? <Button as={Link} icon={Plus} to="/admin/flights/create">Tạo chuyến bay</Button>
    : masterResource
      ? <Button icon={Plus} onClick={() => setEditingItem(null)}>Thêm mới</Button>
      : null;
