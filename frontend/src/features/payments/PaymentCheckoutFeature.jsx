...
const paymentMethods = [
  ...
  { id: "momo", title: "Ví MoMo", description: "Thanh toán qua ví điện tử", icon: Wallet },
  {
    id: "stripe",
    title: "Thẻ quốc tế",
    description: "Được xử lý bởi cổng thanh toán",
    icon: CreditCard,
  },
  {
    id: "cash",
    title: "Thanh toán tiền mặt",
    description: "Theo chính sách hãng bay",
    icon: Landmark,
  },
];
...
export default function PaymentCheckoutFeature({ bookingId }) {
  ...
  const loadPaymentData = useCallback(async () => {
    try {
      ...
      const methods = paymentMethods.filter((method) =>
        (configResponse.data?.providers ?? []).includes(method.id),
      );
  ...
  if (error || !booking) {
    return (
      <div className="mx-auto max-w-4xl px-container-padding py-section-gap">
        <ErrorMessage message={error} onRetry={loadPaymentData} />
      </div>
    );
  }
  ...
  const holdSeconds = booking.hold_expires_at
    ? Math.max(0, Math.ceil((new Date(booking.hold_expires_at).getTime() - now) / 1000))
    : 0;
  ...
  return (
    <main className="min-h-screen bg-background px-container-padding py-section-gap text-on-background">
      <div className="mx-auto max-w-6xl">
        <header className="mb-stack-lg flex flex-col justify-between gap-4 border-b border-outline-variant pb-stack-md sm:flex-row sm:items-center">
        ...
        <div className="mb-stack-lg">
          <h1 className="text-headline-lg font-headline-lg text-primary">Thanh toán an toàn</h1>
          <p className="mt-2 text-body-md text-on-surface-variant">
            Tạo yêu cầu thanh toán cho mã đặt chỗ{" "}
            <span className="font-data-mono text-on-surface">{bookingId}</span>.
          </p>
        </div>
        {error ? (
          <div className="mb-stack-lg">
        ...
        <div className="grid grid-cols-1 items-start gap-stack-lg lg:grid-cols-12">
          <section className="rounded-xl border border-surface-variant bg-surface-container-lowest p-stack-lg shadow-sm lg:col-span-7">
            <h2 className="text-title-lg font-title-lg text-primary">Phương thức thanh toán</h2>
            {booking.status === "pending" ? (
              <p
                className={cn(
                  "mt-3 rounded-lg p-3 text-body-sm font-semibold",
                  holdExpired
              ...
              >
                {holdExpired
                  ? "Đã hết thời gian giữ ghế. Vui lòng tạo booking mới."
                  : `Ghế được giữ thêm ${holdTime}.`}
              </p>
            ) : null}
            <div className="mt-stack-md grid gap-3 sm:grid-cols-2">
              {availableMethods.map((method) => (
                <button
                  ...
                  disabled={Boolean(payment) || holdExpired}
                  key={method.id}
            ...
            <div className="mt-stack-md flex items-start gap-2 rounded-lg bg-primary-fixed p-3 text-body-sm text-on-primary-fixed">
              <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" />
              Số tiền và trạng thái thanh toán được xác nhận ở máy chủ. Website không lưu dữ liệu
              thẻ.
            </div>
            {payment ? (
              <PaymentStatus payment={payment} onRefresh={loadPaymentData} />
            ) : (
              <button
                className="mt-stack-lg inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-status-warning px-5 text-label-md font-label-md font-bold text-primary disabled:opacity-50"
                disabled={
            ...
            {isPaymentFailed ? (
              <div className="mt-4 rounded-lg bg-status-error/10 p-3 text-status-error">
...
function PaymentStatus({ payment, onRefresh }) {
  const instruction =
    payment.provider === "cash"
      ? "Vui lòng thanh toán tại quầy/đại lý. Nhân viên sẽ xác nhận giao dịch trên hệ thống."
      : "Hãy hoàn tất ở cổng thanh toán của nhà cung cấp. Trạng thái sẽ tự cập nhật.";
  return (
    <div className="mt-stack-lg rounded-lg border border-outline-variant bg-surface p-stack-md">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-label-md text-on-surface-variant">Mã giao dịch</p>
          <p className="mt-1 break-all font-data-mono text-body-md text-on-surface">
...
function BookingSummary({ booking, flight }) {
  return (
    <aside className="rounded-xl border border-surface-variant bg-surface-container-lowest p-stack-lg shadow-sm lg:col-span-5 lg:sticky lg:top-8">
      <h2 className="text-title-lg font-title-lg text-primary">Tóm tắt chuyến bay</h2>
      <div className="mt-stack-md border-y border-outline-variant py-stack-md">
        <div className="flex items-center justify-between">
          <div className="text-center">
            <p className="text-headline-lg text-on-surface">{flight?.origin_airport?.code}</p>
            <p className="text-body-sm text-on-surface-variant">
              {formatTime(flight?.departure_time)}
            </p>
          </div>
          <div className="px-4 text-center text-body-sm text-on-surface-variant">
            {flight?.flight_number}
            <br />
            {formatDateTime(flight?.departure_time)}
          </div>
          <div className="text-center">
            <p className="text-headline-lg text-on-surface">{flight?.destination_airport?.code}</p>
            <p className="text-body-sm text-on-surface-variant">
      ...
      <div className="mt-stack-md flex items-end justify-between">
        <span className="text-title-lg text-primary">Tổng thanh toán</span>
