...
export default function FlightDetailPage() {
  ...
  useEffect(() => {
    const loadFlight = async () => {
      ...
      try {
        const response = await flightService.getById(flightId);
        setFlight(toFlightView(response.data));
      } catch (requestError) {
    ...
    <main className="bg-surface-container">
      ...
      <section className="mx-auto grid max-w-7xl grid-cols-1 gap-stack-lg px-container-padding py-stack-lg lg:grid-cols-3">
        ...
        <aside className="rounded-lg border border-surface-container-high bg-surface-container-lowest p-stack-lg shadow-sm">
          ...
          <Button as={Link} className="w-full bg-primary text-white hover:bg-primary/90 [&_svg]:text-white" icon={ArrowRight} to={`/booking/${flight.id}`}>
            Đặt chuyến bay
          </Button>
        </aside>
      </section>
