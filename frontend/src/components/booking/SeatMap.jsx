...
const seatBaseClass =
  "relative flex h-12 items-center justify-center overflow-hidden rounded-b-sm rounded-t-lg border font-data-mono text-[13px] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/20";
...
export default function SeatMap({ seats, selectedSeats = [], onSelectSeat }) {
  ...
  const hasSixSeatsPerRow = seats.some((seat) =>
    ["E", "F"].includes(getSeatParts(seat.seat_number).column),
  );
  ...
  if (sortedRows.length === 0) {
    return (
      <section className="rounded-xl border border-outline-variant bg-surface-container-lowest p-stack-lg text-center text-body-md text-on-surface-variant">
        Chưa có sơ đồ ghế cho chuyến bay này.
      </section>
    );
  }
  ...
  return (
    <section className="rounded-[32px] border border-surface-variant bg-surface-container-low p-4 shadow-[inset_0_4px_10px_rgba(0,0,0,0.03)] md:p-8">
      <div className="mx-auto max-w-xl rounded-t-[72px] border border-outline-variant bg-surface-container-lowest p-4 shadow-sm sm:p-6">
        <div className="mb-5 flex items-center justify-between px-2 text-label-md font-label-md text-on-surface-variant">
          <span>Đầu máy bay</span>
          <span>Lối đi ở giữa</span>
        </div>
        <div className="space-y-3">
          {sortedRows.map(([rowNumber, rowSeats]) => (
            <div
              className="grid grid-cols-[repeat(3,minmax(0,1fr))_2rem_repeat(3,minmax(0,1fr))] gap-2"
              key={rowNumber}
            >
              {columns.left.map((column, index) => (
                <SeatCell
                  isSelected={selectedSeats.some((item) => item.id === rowSeats.get(column)?.id)}
                  key={`${rowNumber}-left-${index}`}
                  onSelectSeat={onSelectSeat}
                  seat={rowSeats.get(column)}
                />
              ))}
              <div className="flex items-center justify-center font-data-mono text-xs text-outline">
                {rowNumber}
              </div>
              {columns.right.map((column, index) => (
                <SeatCell
                  isSelected={selectedSeats.some((item) => item.id === rowSeats.get(column)?.id)}
                  key={`${rowNumber}-right-${index}`}
                  onSelectSeat={onSelectSeat}
                  seat={rowSeats.get(column)}
                />
              ))}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected &&
          "z-10 scale-105 border-2 border-status-warning bg-status-warning text-primary shadow-[0_4px_12px_rgba(251,191,36,0.4)]",
        isUnavailable &&
          "cursor-not-allowed border-surface-variant bg-surface-variant text-outline",
      )}
